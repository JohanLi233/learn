...
def fetch_latest_news():
    ...
    while not duplicate_found and total_saved < 20:
        ...
        if not json_data:
            logging.warning(f"无法获取第 {page} 页的数据，结束本次抓取。")
            break
        ...
        for news in news_list:
            ...
            if news_exists(news["link"]):
                logging.info(f"发现重复新闻，停止抓取: {news['title']}")
    ...
    logging.info(f"本次抓取完成。共保存 {total_saved} 条新闻。")
    return total_saved

def main():
    while True:
        logging.info("开始新一轮的新闻抓取")
        total_saved = fetch_latest_news()
        logging.info(f"本轮抓取结束，共保存 {total_saved} 条新闻。等待下一小时...")
        time.sleep(600)  # 等待一小时
